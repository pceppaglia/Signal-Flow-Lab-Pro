...
var configurations = mysqlTable("configurations", {
  id: int("id").autoincrement().primaryKey(),
...
var configFeedback = mysqlTable("configFeedback", {
  id: int("id").autoincrement().primaryKey(),
...
async function getUserConfigurations(userId) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(configurations).where(eq(configurations.userId, userId)).orderBy(desc(configurations.updatedAt));
}
...
async function updateConfiguration(id, userId, data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const updateSet = {};
  if (data.title !== void 0) updateSet.title = data.title;
  if (data.description !== void 0) updateSet.description = data.description;
  if (data.data !== void 0) updateSet.data = data.data;
  if (data.isPublic !== void 0) updateSet.isPublic = data.isPublic;
  await db.update(configurations).set(updateSet).where(and(eq(configurations.id, id), eq(configurations.userId, userId)));
}
async function deleteConfiguration(id, userId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(configurations).where(and(eq(configurations.id, id), eq(configurations.userId, userId)));
}
async function getPublicConfigurations() {
  const db = await getDb();
  if (!db) return [];
  return db.select({
  ...
  }).from(configurations).where(eq(configurations.isPublic, 1)).orderBy(desc(configurations.updatedAt)).limit(50);
}
async function getUserProgress(userId) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(userProgress).where(eq(userProgress.userId, userId)).orderBy(desc(userProgress.updatedAt));
}
async function getProgressForScenario(userId, scenarioId) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(userProgress).where(and(eq(userProgress.userId, userId), eq(userProgress.scenarioId, scenarioId))).limit(1);
  return result[0];
}
...
async function getConfigFeedback(configId) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(configFeedback).where(eq(configFeedback.configId, configId)).orderBy(desc(configFeedback.createdAt));
}
async function addConfigFeedback(data) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
var appRouter = router({
  ...
  config: router({
    ...
    save: protectedProcedure.input(z2.object({
      ...
      data: z2.any(),
      isPublic: z2.number().optional(),
      scenarioId: z2.string().optional()
  ...
  progress: router({
    ...
    get: protectedProcedure.input(z2.object({ scenarioId: z2.string() })).query(async ({ ctx, input }) => {
    ...
    upsert: protectedProcedure.input(z2.object({
      ...
      progressData: z2.any().optional(),
      completedAt: z2.date().nullable().optional()
    })).mutation(async ({ ctx, input }) => {
  ...
  // ─── Feedback ────────────────────────────────────────────────
  feedback: router({
    list: protectedProcedure.input(z2.object({ configId: z2.number() })).query(async ({ input }) => {
  ...
  assistant: router({
    analyze: protectedProcedure.input(z2.object({
      ...
      cables: z2.array(z2.any()),
      question: z2.string().optional()
    })).mutation(async ({ input }) => {
      const nodesSummary = input.nodes.map((n) => ({
        defId: n.defId,
        settings: n.settings,
        signalLevels: n.signalLevels,
        isClipping: n.isClipping
      }));
      const cablesSummary = input.cables.map((c) => ({
...
Analyze the routing configuration provided and give specific, actionable feedback. Be encouraging but point out issues. Use technical terminology but explain it when needed. Keep responses concise (2-3 paragraphs max).`;
      const userMessage = input.question ? `Here is my current routing setup:

Nodes: ${JSON.stringify(nodesSummary, null, 2)}

Cables: ${JSON.stringify(cablesSummary, null, 2)}

My question: ${input.question}` : `Please analyze my current routing setup and provide feedback on signal flow, gain staging, and any potential issues:
